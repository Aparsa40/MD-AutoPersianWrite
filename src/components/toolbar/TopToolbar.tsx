      ...
      <header className="z-40 flex h-14 shrink-0 items-center justify-between border-b border-border bg-surface px-4 shadow-sm">
        <div className="relative flex flex-row-reverse items-center space-x-2 space-x-reverse">
          ...
          <div className="relative">
            ...
            {activeMenu === 'themes' && <div className="absolute right-0 z-50 mt-2 w-60 rounded border border-border bg-surface py-1 shadow-lg">{themes.map((item) => <button type="button" key={item.id} onClick={() => { setTheme(item.id); closeMenu(); }} className={`w-full px-4 py-2 text-right text-sm hover:bg-bg ${theme === item.id ? 'font-bold text-primary' : ''}`}>{item.label}</button>)}</div>}
          </div>
          <DocumentStyleMenu />
          <AgentMenu />
