...
export const TopToolbar: React.FC = () => {
  ...
  const handleRefresh = async () => {
    ...
    try { await refreshOpenSessions(); } finally { setIsRefreshing(false); closeMenu(); }
  };

  const handleOpenFile = (event: React.ChangeEvent<HTMLInputElement>) => { const file = event.target.files?.[0]; if (!file) return; const reader = new FileReader(); reader.onload = () => { const content = typeof reader.result === 'string' ? reader.result : ''; createSession({ fileName: file.name, markdown: content, isDirty: false, fileHandle: null, workspaceDirectory: null, isWorkspaceFile: false, isNewWorkspaceFile: false }); closeMenu(); }; reader.onerror = () => window.alert('خواندن فایل انجام نشد.'); reader.readAsText(file); event.target.value = ''; };
  const handleSaveAs = async () => { const picker = getSaveFilePicker(); if (!picker) { window.alert('مرورگر فعلی از پنجره انتخاب محل ذخیره فایل پشتیبانی نمی‌کند.'); closeMenu(); return; } try { const handle = await picker({ suggestedName: fileName?.trim() || 'document.md', startIn: activeSession?.workspaceDirectory ?? undefined }); await writeTextFile(handle, markdown); if (activeSession) { updateSession(activeSession.id, { fileHandle: handle, fileName: handle.name, isWorkspaceFile: true, isNewWorkspaceFile: false, isDirty: false }); useEditorStore.setState({ fileName: handle.name, isDirty: false }); markPersisted(handle); } closeMenu(); } catch (error) { if (error instanceof DOMException && error.name === 'AbortError') { closeMenu(); return; } window.alert(error instanceof Error ? error.message : 'ذخیره با نام انجام نشد.'); closeMenu(); } };
  const handleDeleteDocument = async () => { if (!activeSession) { window.alert('سندی برای حذف انتخاب نشده است.'); closeMenu(); return; } const hasPersistedFile = Boolean(activeSession.fileHandle && activeSession.workspaceDirectory); const confirmation = hasPersistedFile ? `آیا از حذف «${activeSession.fileName}» از Workspace و بستن آن مطمئن هستید؟` : `آیا از حذف «${activeSession.fileName}» از برنامه و بستن آن مطمئن هستید؟`; if (!window.confirm(confirmation)) { closeMenu(); return; } try { if (hasPersistedFile && activeSession.workspaceDirectory) await deleteEntry(activeSession.workspaceDirectory, activeSession.fileName, false); closeSession(activeSession.id); closeMenu(); } catch (error) { window.alert(error instanceof Error ? error.message : 'حذف فایل انجام نشد.'); } };
  const handleInsertTableOfContents = () => { const toc = buildTableOfContents(markdown); if (!toc) { window.alert('برای ساخت فهرست مطالب، ابتدا حداقل یک عنوان Markdown مانند # عنوان در سند قرار دهید.'); return; } insertTextAtCursor('', '', toc); closeMenu(); };
  ...
  return (<>
    ...
    <header className="z-40 flex h-14 shrink-0 items-center justify-between border-b border-border bg-surface px-4 shadow-sm">
      <div className="relative flex items-center space-x-2 space-x-reverse">
        <div className="relative"><button type="button" onClick={() => setActiveMenu(activeMenu === 'file' ? null : 'file')} className="rounded px-3 py-1.5 text-sm font-medium hover:bg-bg">فایل</button>{activeMenu === 'file' && <div className="absolute right-0 mt-2 w-56 rounded border border-border bg-surface py-1 shadow-lg">
        ...
        <WorkspaceMenu />
        <div className="relative"><button type="button" onClick={() => setActiveMenu(activeMenu === 'edit' ? null : 'edit')} className="rounded px-3 py-1.5 text-sm font-medium hover:bg-bg">ویرایش</button>{activeMenu === 'edit' && <div className="absolute right-0 mt-2 w-60 space-y-2 rounded border border-border bg-surface p-2 shadow-lg">
        ...
        <div className="relative"><button type="button" onClick={() => setActiveMenu(activeMenu === 'preview' ? null : 'preview')} className="rounded px-3 py-1.5 text-sm font-medium hover:bg-bg">پیش‌نمایش</button>{activeMenu === 'preview' && <div className="absolute right-0 mt-2 w-60 rounded border border-border bg-surface py-1 shadow-lg"><button type="button" onClick={() => { setViewMode('split'); closeMenu(); }} className={`w-full px-4 py-2 text-right text-sm hover:bg-bg ${viewMode === 'split' ? 'font-bold text-primary' : ''}`}>حالت پیش‌فرض (دو پنل)</button><button type="button" onClick={() => { setViewMode('preview-only'); closeMenu(); }} className={`w-full px-4 py-2 text-right text-sm hover:bg-bg ${viewMode === 'preview-only' ? 'font-bold text-primary' : ''}`}>نمایش کامل پیش‌نمایش</button><button type="button" onClick={() => { setViewMode('editor-only'); closeMenu(); }} className={`w-full px-4 py-2 text-right text-sm hover:bg-bg ${viewMode === 'editor-only' ? 'font-bold text-primary' : ''}`}>نمایش کامل ویرایشگر</button><div className="my-1 border-t border-border" /><button type="button" onClick={() => { setOrientation('horizontal'); setViewMode('split'); closeMenu(); }} className={`w-full px-4 py-2 text-right text-sm hover:bg-bg ${orientation === 'horizontal' ? 'font-bold text-primary' : ''}`}>چینش افقی (کنار هم)</button><button type="button" onClick={() => { setOrientation('vertical'); setViewMode('split'); closeMenu(); }} className={`w-full px-4 py-2 text-right text-sm hover:bg-bg ${orientation === 'vertical' ? 'font-bold text-primary' : ''}`}>چینش عمودی (بالا و پایین)</button></div>}</div>
        ...
        <div className="relative"><button type="button" onClick={() => setActiveMenu(activeMenu === 'themes' ? null : 'themes')} className="rounded px-3 py-1.5 text-sm font-medium hover:bg-bg">تم‌ها</button>{activeMenu === 'themes' && <div className="absolute right-0 mt-2 w-56 rounded border border-border bg-surface py-1 shadow-lg">{(['light','dark','sepia','black-white','navy-white'] as const).map((name) => <button type="button" key={name} onClick={() => { setTheme(name); closeMenu(); }} className={`w-full px-4 py-2 text-right text-sm hover:bg-bg ${theme === name ? 'font-bold text-primary' : ''}`}>{name === 'light' ? 'روشن (Light)' : name === 'dark' ? 'تاریک (Dark)' : name === 'sepia' ? 'سپیا (Sepia)' : name === 'black-white' ? 'سیاه و سفید (Black & White)' : 'سرمه‌ای و سفید (Navy & White)'}</button>)}</div>}</div>
        <DocumentStyleMenu /><AgentMenu />
        <div className="relative"><button type="button" onClick={() => setActiveMenu(activeMenu === 'help' ? null : 'help')} className="rounded px-3 py-1.5 text-sm font-medium hover:bg-bg">راهنما / Help</button>{activeMenu === 'help' && <div className="absolute right-0 mt-2 w-48 rounded border border-border bg-surface py-1 shadow-lg"><button type="button" onClick={() => { setIsFeedbackOpen(true); closeMenu(); }} className="w-full px-4 py-2 text-right text-sm hover:bg-bg">ارسال نظر / باگ</button><button type="button" onClick={() => { setIsAboutOpen(true); closeMenu(); }} className="w-full px-4 py-2 text-right text-sm hover:bg-bg">راهنمای برنامه</button></div>}</div>
      </div><div className="text-xs text-text-muted dir-ltr">{fileName}</div>
    </header>
    <FeedbackModal isOpen={isFeedbackOpen} onClose={() => setIsFeedbackOpen(false)} /><AboutModal isOpen={isAboutOpen} onClose={() => setIsAboutOpen(false)} /><HyperlinkModal isOpen={isHyperlinkOpen} onClose={() => setIsHyperlinkOpen(false)} />
  </>);
};