      ...
      <header className="z-40 flex h-14 shrink-0 items-center justify-between border-b border-border bg-surface px-4 shadow-sm">
        <div className="relative flex items-center space-x-2 space-x-reverse">
          ...
          <div className="relative">
            ...
            {activeMenu === 'edit' && (
              <div className="absolute right-0 z-50 mt-2 w-60 space-y-2 rounded border border-border bg-surface p-2 shadow-lg">
                <label className="block text-xs text-text-muted">اندازه فونت:
                  <select value={fontSize} onChange={(e) => setFontSize(Number(e.target.value))} className="mt-1 w-full rounded border border-border bg-bg p-1 text-xs">
                    {[12, 14, 16, 18, 20].map((size) => <option key={size} value={size}>{size}px{size === 16 ? ' (پیش‌فرض)' : ''}</option>)}
                  </select>
                </label>
                <label className="block text-xs text-text-muted">فونت:
                ...
                <button type="button" onClick={() => { setIsHyperlinkOpen(true); closeMenu(); }} className="w-full rounded px-2 py-1.5 text-right text-sm hover:bg-bg">🔗 هایپرلینک</button>
                <button type="button" onClick={handleInsertTableOfContents} className="w-full rounded px-2 py-1.5 text-right text-sm hover:bg-bg">📑 درج فهرست مطالب</button>
          ...
          <div className="relative">
            ...
            {activeMenu === 'preview' && (
              <div className="absolute right-0 z-50 mt-2 w-60 rounded border border-border bg-surface py-1 shadow-lg">
          ...
          <div className="relative">
            ...
            {activeMenu === 'themes' && (
              <div className="absolute right-0 z-50 mt-2 w-56 rounded border border-border bg-surface py-1 shadow-lg">
                {(['light', 'dark', 'sepia', 'black-white', 'navy-white'] as const).map((name) => (
                  <button type="button" key={name} onClick={() => { setTheme(name); closeMenu(); }} className={`w-full px-4 py-2 text-right text-sm hover:bg-bg ${theme === name ? 'font-bold text-primary' : ''}`}>
                    {name === 'light' ? 'روشن (Light)' : name === 'dark' ? 'تاریک (Dark)' : name === 'sepia' ? 'سپیا (Sepia)' : name === 'black-white' ? 'سیاه و سفید (Black & White)' : 'سرمه‌ای و سفید (Navy & White)'}
                  </button>
                ))}
              </div>
            )}
          ...
          <div className="relative">
            ...
            {activeMenu === 'help' && (
              <div className="absolute right-0 z-50 mt-2 w-48 rounded border border-border bg-surface py-1 shadow-lg">
