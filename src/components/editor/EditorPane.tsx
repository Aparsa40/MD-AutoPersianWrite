...
export const EditorPane: React.FC<EditorPaneProps> = ({ editorRef }) => {
  ...
  return (
    <div className="relative h-full w-full overflow-hidden">
      <textarea ref={textareaRef} data-editor="markdown" value={markdown} onChange={(event) => setMarkdown(event.target.value)} onPaste={handlePaste} dir="auto" placeholder="متن مارک‌داون خود را اینجا بنویسید..." className="h-full w-full resize-none overflow-y-auto bg-transparent p-6 leading-relaxed outline-none custom-scrollbar" style={{ fontSize: `${fontSize}px`, fontFamily, color: textColor, unicodeBidi: 'plaintext', textAlign: 'initial' }} />
    </div>
  );
};
