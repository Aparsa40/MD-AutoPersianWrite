...
export const WorkspaceExplorer: React.FC = () => {
  ...
  const activeCloudProviderId = useCloudStore(
    (state) => state.activeProviderId,
  );

  const sessions = useDocumentSessionStore(
    (state) => state.sessions,
  );

  const activeSessionId = useDocumentSessionStore(
    (state) => state.activeSessionId,
  );

  const createSession = useDocumentSessionStore(
    (state) => state.createSession,
  );

  const activateSession = useDocumentSessionStore(
    (state) => state.activateSession,
  );

  const markPersisted = useDocumentSessionStore(
    (state) => state.markPersisted,
  );

  const syncWorkspaceRename = useDocumentSessionStore(
    (state) => state.syncWorkspaceRename,
  );

  const closeWorkspaceSessions = useDocumentSessionStore(
    (state) => state.closeWorkspaceSessions,
  );
  ...
  const provider = useMemo<WorkspaceProvider | null>(() => {
    ...
    const providerId =
      activeWorkspace.providerId ?? activeCloudProviderId;
  ...
  const refresh = useCallback(async () => {
    if (!provider) {
      setEntries([]);
      setError(
        activeWorkspace?.type === 'cloud'
          ? 'اتصال فضای ابری در این نشست مرورگر فعال نیست. لطفاً دوباره متصل شوید.'
          : null,
      );
      return;
    }
  ...
  const promptName = useCallback(
    (message: string, initial = '') =>
      window.prompt(message, initial)?.trim() || null,
    [],
  );
  ...
  const makeReference = useCallback(
    (entry: WorkspaceEntry) => ({
      providerId:
        activeWorkspace!.providerId ??
        (activeWorkspace!.type === 'local'
          ? 'local'
          : activeCloudProviderId ?? 'cloud'),
  ...
  const createFolder = useCallback(async () => {
    ...
    try {
      if (entries.some((entry) => entry.name === name)) {
        throw new Error(
          `«${name}» از قبل در این پوشه وجود دارد.`,
        );
      }
      ...
      reportError(cause, 'ساخت پوشه انجام نشد.');
    }
  }, [
    provider,
    promptName,
  ...
  const createFile = useCallback(async () => {
    ...
    try {
      ...
      reportError(cause, 'ساخت فایل انجام نشد.');
    }
  }, [
    provider,
    promptName,
  ...
  const insertFile = useCallback(async () => {
    ...
    if (!provider || !picker.showOpenFilePicker) return;
  ...
  const importDirectory = useCallback(
    async (
      ...
      for await (const [, item] of getDirectoryEntries(
        handle,
      )) {
        if (item.kind === 'file') {
          await provider.createFile(
  ...
  const insertFolder = useCallback(async () => {
    const picker = pickerWindow();

    if (!provider || !picker.showDirectoryPicker) return;

    try {
      const handle =
        await picker.showDirectoryPicker();

      await importDirectory(handle, parentId);
      await refresh();
    } catch (cause) {
      if (
        cause instanceof DOMException &&
    ...
    async (entry: WorkspaceEntry) => {
      ...
      try {
        const content = decodeText(
          await provider.readFile(entry.id),
        );
        ...
        const existing = sessions.find(
          (session) =>
            session.workspaceFile?.entryId === entry.id &&
            session.workspaceFile?.providerId ===
              providerId &&
            session.workspaceFile?.workspaceId ===
              activeWorkspace?.id,
        );
    ...
    async (entry: WorkspaceEntry) => {
      ...
      try {
        ...
        if (
          conflict &&
          !window.confirm(
            `«${name}» از قبل وجود دارد. جایگزین شود؟`,
          )
        ) {
          return;
        }
  ...
  const deleteEntry = useCallback(
    async (entry: WorkspaceEntry) => {
      ...
      try {
        ...
        reportError(
          cause,
          'حذف انجام نشد. بررسی مجوز دسترسی را انجام دهید.',
        );
      }
    },
    [
  ...
  const isInvalidMoveTarget = useCallback(
    (
      ...
      if (
        ...
        return source.id === targetParentId;
      }
      ...
      if (activeWorkspace?.type === 'local') {
        return targetParentId.startsWith(
          `${source.id}/`,
        );
      }
  ...
  const destinationExists = useCallback(
    (
      targetParentId: string | null,
      name: string,
    ) =>
  ...
  const pasteEntry = useCallback(
    async (target?: WorkspaceEntry) => {
      ...
      if (
        ...
        setError(
          'امکان انتقال یک پوشه به خودش یا یکی از زیرپوشه‌هایش وجود ندارد.',
        );
        return;
      }
      ...
      try {
        if (
          ...
          !window.confirm(
            `«${clipboard.entry.name}» در مقصد وجود دارد. عملیات ادامه پیدا کند؟`,
          )
        ) {
          return;
        }
  ...
  const action = useCallback(
    async (
      entry: WorkspaceEntry,
      operation: WorkspaceEntryAction,
    ) => {
      if (operation === 'copy') {
  ...
  const handleKeyDown = useCallback(
    (event: KeyboardEvent) => {
      ...
      if (
        ...
        void renameEntry(selectedEntries[0]);
      } else if (
        event.key === 'Delete'
      ) {
        event.preventDefault();
        void Promise.all(
          selectedEntries.map(deleteEntry),
        );
      } else if (
        ...
        event.preventDefault();
        copyToClipboard(selectedEntries[0]);
      } else if (
        (event.ctrlKey || event.metaKey) &&
        event.key.toLowerCase() === 'x'
      ) {
        event.preventDefault();
        cutToClipboard(selectedEntries[0]);
      } else if (
        (event.ctrlKey || event.metaKey) &&
  ...
  const handleDragStart = useCallback(
    (
      ...
      event.dataTransfer.setData(
        'text/x-md-autopersianwrite-entry',
        entry.id,
      );
  ...
  const handleFolderHover = useCallback(
    async (entry: WorkspaceEntry) => {
      ...
      try {
        const children =
          await provider.list(entry.id);

        setChildCounts((current) => ({
          ...current,
          [entry.id]: children.length,
        }));
  ...
  if (!activeWorkspace) {
    return (
      <div className="flex flex-1 items-center justify-center p-6 text-center text-sm text-text-muted">
        برای نمایش Workspace، ابتدا یک Local یا Cloud Workspace انتخاب یا ایجاد کنید.
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      >
        ...
        {error && (
          <div
            className="mb-2 rounded-md border border-red-400/40 bg-red-500/10 px-3 py-2 text-xs text-red-700 dark:text-red-200"
            role="alert"
          >
            {error}
          </div>
        ...
        {!loading &&
          !error &&
          entries.length === 0 && (
            <div className="rounded-md border border-dashed border-border p-6 text-center text-xs text-text-muted">
              این پوشه خالی است.
            </div>
          )}
        ...
        <div className="space-y-0.5">
          {entries.map((entry) => (
            ...
            >
              ...
              {hoveredFolder ===
                entry.id &&
                hoverPreview.length > 0 && (
                  <div className="pointer-events-none mx-2 rounded-md border border-border bg-surface p-2 text-[11px] shadow-md">
                    ...
                    {hoverPreview.map(
                      (child) => (
                        <div
                          key={child.id}
                          className="truncate py-0.5 text-text-muted"
                        >
                          {child.type ===
                          'folder'
                            ? '📁'
      ...
      {clipboard && (
        <div className="shrink-0 border-t border-border px-3 py-1.5 text-right text-[10px] text-text-muted">
          {clipboard.cut ? '✂' : '⧉'}{' '}
          {clipboard.entry.name} در Clipboard
          Workspace قرار دارد؛ P برای Paste.
        </div>
      )}

      {activeSessionId && markdown && (
        <button
          type="button"
          onClick={async () => {
            const session =
          ...
          className="shrink-0 border-t border-border px-3 py-2 text-right text-xs hover:bg-bg"
        >
          ذخیره تغییرات فایل فعال
        </button>
      )}

      {context && (
        <div
          className="fixed z-[100] min-w-44 rounded-md border border-border bg-surface p-1 shadow-xl"
          style={{
            left: context.x,
            top: context.y,
          }}
          ...
          {context.entry ? (
            <>
              <button
                ...
                className="block w-full rounded px-3 py-2 text-right text-xs hover:bg-bg"
              >
                باز کردن
              </button>
              ...
              <button
                ...
                className="block w-full rounded px-3 py-2 text-right text-xs text-red-600 hover:bg-bg"
              >
                حذف (Delete)
              </button>
            </>
