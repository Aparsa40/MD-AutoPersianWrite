...
- **تازه‌سازی برنامه و اسناد:** بازخوانی محتوای فایل‌های Workspace باز از طریق File menu و به‌روزرسانی sessionهای مربوط.

## 🆕 تغییرات نسخه 2.5.1

- تثبیت مسیر رندر Mermaid و جلوگیری از تغییر محتوای خام Mermaid توسط `rehype-prism-plus`.
- حفظ منبع اصلی Mermaid روی HAST code node و استفاده از آن در `PreviewPane` برای جلوگیری از خطاهایی مانند `Syntax error in text` و `[object Object]`.
- حفظ Syntax Highlighting برای زبان‌های معمولی و اعمال حفاظت اختصاصی برای Mermaid.
- اضافه شدن گزینه **تازه‌سازی برنامه و اسناد** به منوی File برای بازخوانی محتوای فایل‌های Workspace باز.
- حفظ Mermaid در پروژه؛ قابلیت Refresh بدون حذف dependency یا rendering Mermaid اضافه شده است.
- اجرای دوباره اعتبارسنجی CI و Vercel پس از به‌روزرسانی نسخه و `package-lock.json`.
